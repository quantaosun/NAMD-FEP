...
# Everything below is a default, overwritten from system.ini by configure().
# The step count, the lambda schedule and the row counts used to be literals
# here AND in the config generator AND in fep.tcl, which is how a system ends up
# with windows of one length and an assembler expecting another.  There is now
# one source.
...
NWIN = len(LAM) - 1                     # 15
# Production steps per lambda window. 500000 = 1 ns at a 2 fs timestep, giving
# 15 ns per leg. Chosen over the 6I5I run's 250000 (0.5 ns): that run came out
# reversible but NOT converged -- stationarity was 3x the error, carried by the
# complex leg -- and 4YLJ is the larger system.
DEFAULT_STEPS = 500000
# alchOutFreq is written as 500 by write_fep_inputs.py, so this is derived
# rather than repeated: a window's row count follows from its step count, and
# hardcoding both is how `assemble` starts rejecting complete windows as
# truncated.
EXPECT_LINES = DEFAULT_STEPS // 500
...
STAGES = ("nvt_equil", "npt_equil", "md_forward", "md_backward")


def configure(cfg) -> None:
    """Take every run parameter from a loaded Config."""
    global BASE, NAMD, FLAGS, LAM, NWIN, DEFAULT_STEPS, EXPECT_LINES, EQUIL_ROWS, LEGS
    BASE = cfg.root
    NAMD = str(cfg.resolve(cfg.get("binaries", "namd")))
    FLAGS = list(cfg.get("binaries", "flags"))
    LAM = cfg.lambdas
    NWIN = cfg.nwin
    DEFAULT_STEPS = cfg.steps_per_window
    EXPECT_LINES = cfg.expect_rows
    EQUIL_ROWS = cfg.equil_rows
    LEGS = tuple(cfg.legs)


def schedule_hash(cfg) -> str:
    """Must agree with rbfe.inputs.schedule_hash -- both derive from the config."""
    import hashlib
    return hashlib.sha256(" ".join(cfg.lambda_tokens).encode()).hexdigest()[:12]


def check_fep_tcl(cfg) -> None:
    """Refuse to run against a fep.tcl that has drifted from the config.

    The per-window driver does not read fep.tcl at all -- it substitutes
    explicit alchLambda/alchLambda2 -- so a stale tcl would only bite the
    single-run path.  That is precisely why it needs checking: the failure would
    be silent, and would look like a normal run.
    """
    from rbfe import inputs as I
    from rbfe.errors import ConfigError
    p = cfg.root / "fep.tcl"
    if not p.exists():
        raise ConfigError(f"{p} missing -- run `rbfe inputs` first.")
    if p.read_text() != I.render_fep_tcl(cfg):
        raise ConfigError(
            f"{p} does not match [run] lambdas in system.ini.\n"
            f"  expected schedule-hash {schedule_hash(cfg)}\n"
            f"  Re-run `rbfe inputs` to regenerate it. (A hand-edited fep.tcl "
            f"with an unchanged config is the case this catches.)")
